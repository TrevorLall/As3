...
require 'inludes/DbTest.php';
	if(isset($_POST['payBtn'])){
		...
		if($insertpay = $conn->query("INSERT INTO `payment_method`(`payment_id`, `credit_type`, `card_number`, `security_code`, `exp_date`, `user_id`, `address_id`, `card_name`) VALUES ('null','{$ctype}','{$cnum}','{$ctype}','{$exp}',17,{$_SESSION["address_id"]},'{$cname}')")){
			Echo "added to address";
		}
		else{
			echo "Error: " . $sql . "<br>" . mysqli_error($conn);		
